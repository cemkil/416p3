...
public class Node
{
    ...
    public Node() {
        ...
        for (int i = 0; i<DVSimulator.NUMNODES; i++) {
            ...
            // BestPath to any d = this node's id, use id
            //            // Else if node is a direct neighbor, use the neighbor id
            //            // Otherwise, choose a random neighbor (see randomNeighbor method)node should be initialized as follows:
            // If node has i
    ...
    public void notifyNeighbors() {
        ...
        for(int i=0 ; i<neighbors.length; i++){
            Packet packet = new Packet(this.id,neighbors[i],myDV);
    ...
    public void updateDV(Packet p) {
        ...
        for (int j = 0; j < neighborDV[neighbor_id].length; j++){
            int min = Math.min(myDV[j], cost[neighbor_id] + neighborDV[neighbor_id][j]);
                if(min < myDV[j]){
                    myDV[j] = min;
                    bestPath[j] = neighbor_id;
                    ischange = true;
                }
        }
        if (ischange){
